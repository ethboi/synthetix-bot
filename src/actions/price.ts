...
import { ETH_OP, BTC_OP, LYRA_OP, THALES_OP, KWENTA_OP, SNX_OP, TLX_OP, PYTH_OP } from '../constants/addresses';
...
export async function GetPrices() {
  ...
  const [pairsEth, pairsBtc, pairsLyra, pairsThales, pairsSNX, pairsKwenta, pairsTLX, pairsPYTH] = await Promise.all([
  ...
  console.log('API responses fetched successfully.');
  ...
  try {
    ...
    const pairLyra = dexLyra.pairs.find((pair) => pair.baseToken.address.toLowerCase() == LYRA_OP.toLowerCase() && pair.baseToken.symbol == 'LYRA');
    if (pairLyra) {
      pairs.push(pairLyra);
    }
