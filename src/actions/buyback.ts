...
import { Buyback, BuybackData } from '../types/buyback'
import { BuyBackResponse } from '../types/kwenta'
...
export async function GetBuybackData() {
  try {
    ...
    const data = response.data as BuyBackResponse

    if (data.status == '1') {
      const buybackData = parseBuybackData(response.data.result as BuybackData[])
